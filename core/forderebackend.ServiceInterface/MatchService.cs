...
namespace forderebackend.ServiceInterface
{
    public class MatchService : BaseService
    {
        ...
        {
            return Db.SingleById<MatchView>(request.Id);
        }

        public object Get(GetMatchesForTeamRequest request)
        {
            return Db.Select(Db.From<MatchView>()
                .Where(x => x.HomeTeamId == request.TeamId || x.GuestTeamId == request.TeamId));
        }
        ...
        public object Get(UpcommingMatchesRequest request)
        {
            ...
            var matches = Db.Select(Db.From<MatchView>().Where(p =>
                p.PlayDate >= today && p.ResultDate == null && p.SeasonId == currentSeason.Id));
        ...
        public object Get(RecentMatchesRequest request)
        {
            ...
            var matches = Db.Select(Db.From<MatchView>().Where(p =>
                p.PlayDate >= sevenDaysBack && p.PlayDate <= today && p.ResultDate != null &&
                p.SeasonId == currentSeason.Id));
        ...
        {
            return Db.Select(Db.From<MatchView>().Where(p => p.LeagueId == request.Id));
        }

        public object Get(GetMatchesByCompetitionRequest request)
        {
            return Db.Select(Db.From<MatchView>().Where(p => p.CompetitionId == request.Id));
        }
        ...
        public object Get(UserMatchesRequest request)
        {
            ...
            return
                Db.Select(
                    Db.From<MatchView>().Where(
                        p => p.GuestPlayer1Id == request.UserId || p.GuestPlayer2Id == request.UserId ||
                             p.HomePlayer1Id == request.UserId || p.HomePlayer2Id == request.UserId));
        }
        ...
        public object Get(MyMatchesTodayRequest request)
        {
            return Db.Select(
                Db.From<MatchView>().Where(
                        p =>
                            (p.GuestPlayer1Id == SessionUserId || p.GuestPlayer2Id == SessionUserId ||
                             p.HomePlayer1Id == SessionUserId || p.HomePlayer2Id == SessionUserId) &&
                            p.PlayDate >= DateTime.UtcNow.Date &&
                            p.PlayDate <= DateTime.UtcNow.AddDays(1).Date &&
                            (p.FinalDayCompetitionId == null || p.FinalDayCompetitionId != null &&
                                p.FinalDayTableNumber != null && !p.IsFreeTicket))
        ...
        {
            var match = Db.LoadSingleById<Match>(request.Id);
            ...
            Db.Update<Match>(new {request.TableId, PlayDate = universalTime, RegisterDate = DateTime.UtcNow},
                p => p.Id == request.Id);

            var dto = Get(new GetMatchByIdRequest {Id = request.Id});
        ...
        public object Delete(DeleteMatchAppointmentRequest request)
        {
            ...
            Db.Update(match, p => p.Id == request.MatchId);

            var dto = Get(new GetMatchByIdRequest {Id = request.MatchId});
        ...
        public object Get(GetUserMailsForOpenMatchesInLeagueRequest request)
        {
            ...
            var teamOfUser = Db.Single<Team>(x =>
                x.LeagueId == request.LeagueId && (x.Player1Id == userId || x.Player2Id == userId));
            var matches = Db.Select<Match>(sql =>
                sql.LeagueId == request.LeagueId && sql.PlayDate == null && sql.RegisterDate == null &&
                (sql.HomeTeamId == teamOfUser.Id || sql.GuestTeamId == teamOfUser.Id));

            var mailList = new List<string>();
            var teamIds = matches.Select(s => s.HomeTeamId).Concat(matches.Select(s => s.GuestTeamId)).Distinct()
                .ToList();

            var teams = Db.SelectByIds<Team>(teamIds).ToDictionary(k => k.Id);
            var userIds = teams.Values.Select(s => s.Player1Id).Concat(teams.Values.Select(s => s.Player2Id)).ToList();
            var users = Db.SelectByIds<UserAuth>(userIds).ToDictionary(k => k.Id);
        ...
        public object Delete(DeleteMatchResultRequest request)
        {
            ...
            StandingsCalculator.Calculate(Db, match.LeagueId);
        ...
        {
            var match = Db.Single<MatchView>(x => x.Id == request.Id);
        ...
        public MatchViewDto Put(UpdateMatchRequest request)
        {
            ...
            StandingsCalculator.Calculate(Db, matchToUpdate.LeagueId);
        ...
        public MatchViewDto Post(ResetMatchRequest request)
        {
            ...
            StandingsCalculator.Calculate(Db, matchToReset.LeagueId);
