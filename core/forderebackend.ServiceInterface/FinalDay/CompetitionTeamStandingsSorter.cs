...
namespace forderebackend.ServiceInterface.FinalDay
{
    ...
    {
        public static IOrderedEnumerable<CompetitionTeamStanding> OrderByRanking(IList<CompetitionTeamStanding> standings, IList<Match> matches)
        {
            var matchResultComparer = new DirectMatchComparar(matches);
            var competitionTeamStandings = standings.OrderByDescending(p => p.Points).ThenByDescending(x => x, matchResultComparer);
        ...
        public class DirectMatchComparar : IComparer<CompetitionTeamStanding>
        {
            ...
            public int Compare(CompetitionTeamStanding x, CompetitionTeamStanding y)
            {
                ...
                var match = matches.Single(sql => (sql.HomeTeamId == x.TeamId && sql.GuestTeamId == y.TeamId) || (sql.HomeTeamId == y.TeamId && sql.GuestTeamId == x.TeamId));
            ...
            {
                var possibleCircleMatches = matches.Where(x => x.WinnerTeamId == loserTeamId && !visitedMatches.Contains(x)).ToList();
