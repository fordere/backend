...
namespace forderebackend.ServiceInterface.FinalDay
{
    ...
    public class SingleKoCompetitionMode : ICompetitionMode
    {
        ...
        private List<Match> GenerateMatchesForGroup(List<TeamInGroup> teams, int finalDayCompetitionId)
        {
            ...
            {
                var homeTeam = orderedTeams.First();
                var guestTeam = orderedTeams.Skip(1).First();
                ...
                var match = new Match
                {
                    HomeTeamId = homeTeam.TeamId, GuestTeamId = guestTeam.TeamId, RoundOrder = counter, CupRound = 1,
        ...
        public List<Match> GenerateMatchAfterMatchResultEntered(Match match)
        {
            ...
            var upcommingMatch = new Match
            {
                HomeTeamId = match.WinnerTeamId, GuestTeamId = correspondingMatch.WinnerTeamId, CupRound = nextCupRound,
                RoundOrder = nextRoundOrder, FinalDayCompetitionId = match.FinalDayCompetitionId
            };

            if (IsFinal(upcommingMatch.FinalDayCompetitionId.Value, upcommingMatch.CupRound))
            {
                var matchPlace3 = new Match
                {
                    HomeTeamId = match.LoserTeamId, GuestTeamId = correspondingMatch.LoserTeamId,
                    CupRound = nextCupRound, RoundOrder = nextRoundOrder + 1,
                    FinalDayCompetitionId = match.FinalDayCompetitionId
                };
        ...
        {
            var numberOfFirstRoundMatches = dbConnection.Count(dbConnection.From<Match>()
                .Where(x => x.FinalDayCompetitionId == finalCompetitionId && x.CupRound == 1));
        ...
        {
            var numberOfMatchesFirstRound = dbConnection.Count<MatchView>(x =>
                x.FinalDayCompetitionId == finalDayCompetitionId && x.CupRound == 1);
        ...
        {
            return dbConnection.Single(dbConnection.From<MatchView>().Where(x =>
                x.CupRound == cupRound && x.RoundOrder == roundOrder &&
                x.FinalDayCompetitionId == finalDayCompetitionId));
        }
        ...
        private void FillWithFreilos(ICollection<TeamInGroup> orderedTeams, int bracketSize)
        {
            while (orderedTeams.Count < bracketSize)
                orderedTeams.Add(new TeamInGroup
                    {TeamId = CreateNewFreilos(), Settlement = orderedTeams.Max(x => x.Settlement) + 1});
        }
        ...
        {
            var bracketSize = 2;
            while (numberOfTeams > bracketSize) bracketSize *= 2;
