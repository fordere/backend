...
namespace forderebackend.ServiceInterface.FinalDay
{
    public class UpcommingMatchSorter
    {
        ...
        public List<MatchView> Sort(IDbConnection dbConnection, List<MatchView> matches, int finalDayId)
        {
            ...
            var sortInfos = matches.Select(x => new SortInfo
                {Match = x, NumberOfOpenTables = tables.Count(t => t.TableType == x.FinalDayTableType)}).ToList();

            foreach (var playerCounter in userOpenGames)
            {
                var playerId = playerCounter.Key;
            ...
            var finalDayCompetitionIds = dbConnection.Select<FinalDayCompetition>(x => x.FinalDayId == finalDayId)
                .Select(x => x.Id).ToList();
            var allPlayedMatches = dbConnection.Select<MatchView>(x =>
                x.ResultDate != null && Sql.In(x.FinalDayCompetitionId, finalDayCompetitionIds)).ToList();
            foreach (var sortInfo in sortInfos)
                sortInfo.LastPlayDate = allPlayedMatches.Where(
                        x =>
                            IsPlayerInMatch(x, sortInfo.Match.HomePlayer1Id) ||
                            IsPlayerInMatch(x, sortInfo.Match.HomePlayer2Id) ||
                            IsPlayerInMatch(x, sortInfo.Match.GuestPlayer1Id) ||
                            IsPlayerInMatch(x, sortInfo.Match.GuestPlayer2Id))
                    .Max(x => x.PlayDate) ?? new DateTime(1970, 1, 1);

            var orderedSortInfos = sortInfos.OrderBy(x => x.Match.FinalDayCompetitionPriority)
                .ThenByDescending(x => x.HasFreeTable).ThenByDescending(x => x.PlayerOpenMatchesMax)
                .ThenByDescending(x => x.PlayerOpenMatchesSum).ThenBy(x => x.NumberOfOpenTables)
                .ThenBy(x => x.LastPlayDate);
        ...
        {
            var allTables =
                dbConnection.Select(dbConnection.From<FinalDayTable>().Where(x => x.FinalDayId == finalDayId));
            var finalDayCompetitionIds = dbConnection
                .Select(dbConnection.From<FinalDayCompetition>().Where(x => x.FinalDayId == finalDayId))
                .Select(x => x.Id).ToList();
            var runningMatches = dbConnection.Select(dbConnection.From<MatchView>().Where(x =>
                x.ResultDate == null && x.PlayDate != null && Sql.In(x.FinalDayCompetitionId, finalDayCompetitionIds)));
        ...
        {
            return matchView.HomePlayer1Id == playerId || matchView.HomePlayer2Id == playerId ||
                   matchView.GuestPlayer1Id == playerId || matchView.GuestPlayer2Id == playerId;
        }
