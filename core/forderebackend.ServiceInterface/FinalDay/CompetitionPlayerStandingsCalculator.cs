...
namespace forderebackend.ServiceInterface.FinalDay
{
    public static class CompetitionPlayerStandingsCalculator
    {
        ...
        {
            var players =
                db.LoadSelect<PlayerInFinalDayCompetition>(x => x.FinalDayCompetitionId == finalDayCompetitionId)
        ...
        private static void UpdateRanks(IDbConnection db, List<CompetitionPlayerStanding> standings)
        {
            ...
            var orderedTableEntries = standings.OrderByDescending(p => p.Points).ThenByDescending(o => o.PlusMinus)
        ...
        private static void UpdateStandings(IDbConnection db, List<UserAuth> players, List<MatchView> matches,
            List<CompetitionPlayerStanding> standings)
        {
            foreach (var player in players)
            {
                var matchesOfTeam = matches.Where(p =>
                    p.HomePlayer1Id == player.Id || p.HomePlayer2Id == player.Id || p.GuestPlayer1Id == player.Id ||
                    p.GuestPlayer2Id == player.Id).ToList();
                ...
                tableEntry.GamesWon = playedMatchesOfTeam.Count(p =>
                    p.WinnerTeamPlayer1 == player.Id || p.WinnerTeamPlayer2 == player.Id);

                tableEntry.GamesDraw = playedMatchesOfTeam.Count(p => p.IsDraw);

                tableEntry.GamesLost = playedMatchesOfTeam.Count(p =>
                    p.WinnerTeamPlayer1 != player.Id && p.WinnerTeamPlayer2 != player.Id && !p.IsDraw);

                tableEntry.GoalsScored = playedMatchesOfTeam
                                             .Where(p => p.HomePlayer1Id == player.Id || p.HomePlayer2Id == player.Id)
                                             .Sum(s => s.HomeTeamScore.GetValueOrDefault())
                                         + playedMatchesOfTeam
                                             .Where(p => p.GuestPlayer1Id == player.Id || p.GuestPlayer2Id == player.Id)
                                             .Sum(s => s.GuestTeamScore.GetValueOrDefault());

                tableEntry.GoalsConceded = playedMatchesOfTeam
                                               .Where(p => p.HomePlayer1Id == player.Id || p.HomePlayer2Id == player.Id)
                                               .Sum(s => s.GuestTeamScore.GetValueOrDefault()) +
                                           playedMatchesOfTeam
                                               .Where(p => p.GuestPlayer1Id == player.Id ||
                                                           p.GuestPlayer2Id == player.Id).Sum(s =>
                                                   s.HomeTeamScore.GetValueOrDefault());
        ...
        private static void EnsureEachPlayerHasTableEntry(IDbConnection db, List<UserAuth> players,
            List<CompetitionPlayerStanding> standings, int finalDayCompetitionId)
        {
            foreach (var player in players)
            {
                ...
                {
                    competitionPlayerStanding = new CompetitionPlayerStanding
                        {PlayerId = player.Id, FinalDayCompetitionId = finalDayCompetitionId};
