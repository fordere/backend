...
namespace Fordere.RestService
{
    ...
    public class CompetitionService : BaseService
    {
        ...
        public object Get(GetCompetitionStateRequest request)
        {
            ...
            var hasNotAssigendTeams = Db.Select<TeamInscription>(t => t.CompetitionId == competitionId && t.AssignedLeagueId == null).Any();
        ...
        public object Get(GetCompetitionsOpenForRegistration request)
        {
            ...
            var seasonsWithActiveRegistrationPeriod = Db.Select(Db.From<Season>().Where(x => x.State == SeasonState.Registration || x.State == SeasonState.PrepareSeason).Where(x => x.DivisionId == DivisionId));

            // Find all competitions for the given seasons
            var competitions = Db.Select<Competition>().Where(x => seasonsWithActiveRegistrationPeriod.Any(t => t.Id == x.SeasonId)).ToList();
        ...
        {
            var teamInscription = Db.Single<TeamInscription>(x => (x.Player1Id == SessionUserId || x.Player2Id == SessionUserId) && x.CompetitionId == competitionId);
        ...
        {
            var competitions = this.Db.LoadSelect<Competition>();
            return competitions.Select(s => s.ConvertTo<CompetitionDto>());
        }

        public object Get(GetAllCompetitionsInSeasonRequest request)
        {
            var competitions = this.Db.LoadSelect<Competition>(sql => sql.SeasonId == request.SeasonId);
            return competitions.Select(s => s.ConvertTo<CompetitionDto>());
        }
        ...
        public object Get(GetStandingsFromSeasonRequest request)
        {
            ...
            competitions.ForEach(x => competitionsDtos.Add((CompetitionDto)Get(new GetStandingsFromCompetitionsRequest { Id = x.Id })));
        ...
        {
            var competition = this.Db.LoadSingleById<Competition>(request.Id);
            ...
            foreach (var league in competition.Leagues.OrderBy(x => x.Number))
            {
                ...
                leagueDto.TableEntries = this.Db.Select(Db.From<TableEntryView>().Where(p => p.LeagueId == league.Id)).Select(s => s.ConvertTo<TableEntryViewDto>()).OrderBy(o => o.Rank).ToList();
        ...
        public object Get(GetTeamsFromCompetitionsRequest request)
        {
            ...
            var allTeams = this.Db.Select(Db.From<TeamView>().Where(p => Sql.In(p.LeagueId, leagueIds)));
            ...
            foreach (var league in competition.Leagues.OrderBy(x => x.Number))
            {
                ...
                leagueDto.Teams = allTeams.Where(p => p.LeagueId == league.Id).Select(s => s.ConvertTo<TeamViewDto>()).ToList();
        ...
        public object Post(CompetitionRegisterRequest request)
        {
            ...
            var newId = this.Db.Insert(entity, true);

            var player1 = this.Db.Single<UserAuth>(x => x.Id == SessionUserId);
            var player2 = this.Db.Single<UserAuth>(x => x.Id == entity.Player2Id);
            var bar = this.Db.Single<Bar>(x => x.Id == entity.BarId);

            CreatePaymentEntry(player1, competition.SeasonId);
            CreatePaymentEntry(player2, competition.SeasonId);

            MailSender.SendRegistrationConfirmation(DivisionId, entity.Name, entity.SeasonAmbition, entity.WishPlayDay, player1, player2, competition, bar);

            return new HttpResult(null, HttpStatusCode.Created)
            {
                Location = this.Request.AbsoluteUri + "/teaminscriptions/" + newId
            };
        }
        ...
        {
            var count = this.Db.Count<TeamInscription>(p => p.CompetitionId == competitionId &&
                                                           (p.Player1Id == player1 ||
                                                            p.Player2Id == player1 ||
                                                            p.Player1Id == player2 ||
                                                            p.Player2Id == player2));
