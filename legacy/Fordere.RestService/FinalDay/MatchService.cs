...
namespace Fordere.RestService.FinalDay
{
    public class MatchService : BaseService
    {
        ...
        public object Get(GetRecentFinalDayMatchesRequest request)
        {
            ...
            {
                var finalDayCompetitionIds = Db.Select(Db.From<FinalDayCompetition>().Where(x => x.FinalDayId == request.FinalDayId && x.State == FinalDayCompetitionState.Running)).Select(x => x.Id);
                var playedMatches = Db.Select(Db.From<MatchView>().Where(x => Sql.In(x.FinalDayCompetitionId, finalDayCompetitionIds) && !x.IsFreeTicket && x.PlayDate != null && x.ResultDate != null).OrderByDescending(x => x.ResultDate).Limit(request.Offset, request.PageSize)).ToList();
        ...
        public object Get(GetUpcommingFinalDayMatchesRequest request)
        {
            ...
            {
                var finalDayCompetitionIds = Db.Select(Db.From<FinalDayCompetition>().Where(x => x.FinalDayId == request.FinalDayId && x.State == FinalDayCompetitionState.Running)).Select(x => x.Id);
                var openMatches = Db.Select(Db.From<MatchView>().Where(x => Sql.In(x.FinalDayCompetitionId, finalDayCompetitionIds) && x.PlayDate == null)).ToList();
                ...
                {
                    sortedUpcommingMatches = sortedUpcommingMatches.Skip(request.Offset).Take(request.PageSize.Value).ToList();
                }
        ...
        public object Get(GetFinishedFinalDayMatchesRequest request)
        {
            Func<MatchesResponse> func = () =>
            {
                var finalDayCompetitionIds = Db.Select(Db.From<FinalDayCompetition>().Where(x => x.FinalDayId == request.FinalDayId)).Select(x => x.Id);
                IEnumerable<MatchView> query = Db.Select(Db.From<MatchView>().Where(x => Sql.In(x.FinalDayCompetitionId, finalDayCompetitionIds) && x.GuestTeamScore != null && x.HomeTeamScore != null));

                if (!string.IsNullOrEmpty(request.TeamFilter))
                {
                    query = query.Where(x => x.HomeTeamName.Contains(request.TeamFilter) || x.GuestTeamName.Contains(request.TeamFilter));
                }
        ...
        {
            var firstMatchViewPlayerIds = new List<int> { firstMatchView.HomePlayer1Id, firstMatchView.HomePlayer2Id, firstMatchView.GuestPlayer1Id, firstMatchView.GuestPlayer2Id };
            var secondMatchViewPlayerIds = new List<int> { secondMatchView.HomePlayer1Id, secondMatchView.HomePlayer2Id, secondMatchView.GuestPlayer1Id, secondMatchView.GuestPlayer2Id };
        ...
        {
            var finalDayCompetitionIds = Db.Select(Db.From<FinalDayCompetition>().Where(x => x.FinalDayId == finalDayId)).Select(x => x.Id);

            return Db.Select(Db.From<MatchView>().Where(x => Sql.In(x.FinalDayCompetitionId, finalDayCompetitionIds) && x.PlayDate != null && x.ResultDate == null)).OrderBy(s => s.FinalDayTableNumber).ToList();
        }
    }
}
